...
(HTTP) and the Kubernetes API on 6551. No Ingress declares TLS, so no
HTTPS port is published. The cluster writes its own kubeconfig
...
six minutes with the images already built. A re-run reconciles in
...
13000 (Forgejo bootstrap), 18080, 18081, 18082, 19090, and 19091 (smoke
test and stack validation), and 19092 (the go-grpc health call in a
scenario's `--verify`).
...
`k8s/infra/monitoring`, and `k8s/infra/monitoring-flux` into that
repository.
...
(`monitoring`, `apps`, `monitoring-flux`), and a `Receiver`.
...
before a `ServiceMonitor` or a `PrometheusRule` will apply.
...
A single PostgreSQL Deployment holds both databases. `rust-inventory`
owns `inventory`, `go-api` owns `orders`. Neither service creates its
own schema. Migration Jobs do this before the services need it, and a
service fails at startup if the schema is missing.
...
- `forgejo.localhost`, `grafana.localhost`, and `prometheus.localhost` to the
  infrastructure
